...
import '../model/all_user_list_model.dart';
...
class AuthService {
  ...
  Future<UserLoginModel> login(String username, String password) async {
    ...
    if (response.statusCode == 200) {
      return UserLoginModel.fromJson(
          json.decode(response.body)); // i forget to decode type of data here
    } else {
  ...
  // Display all user list and their information form dummy json
  Future<AllUserListModel> getAllUserList() async {
    ...
    if (response.statusCode == 200) {
      return AllUserListModel.fromJson(
